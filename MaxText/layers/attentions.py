...
class AttentionOp(nn.Module):
  ...
  def qk_product(self, query: Array, key: Array | aqt_tensor.QTensor, q_seq_len: int, model_mode: str) -> Array:
    ...
    einsum = jnp.einsum
    if isinstance(key, aqt_tensor.QTensor):
      num_bits = 4 if key.qvalue.dtype == jnp.int4 else 8
  ...
  def wv_product(self, attn_weights: Array, value: Array | aqt_tensor.QTensor, model_mode: str) -> Array:
    ...
    einsum = jnp.einsum
    if isinstance(value, aqt_tensor.QTensor):
      num_bits = 4 if value.qvalue.dtype == jnp.int4 else 8
      wv_cfg = aqt_config.dot_general_make(
          lhs_bits=None,
          rhs_bits=num_bits,
          bwd_bits=None,
          use_fwd_quant=False,
      ...
      aqt_config.set_fwd_dequant_mode(
          wv_cfg, rhs_dequant_mode=aqt_config.DequantMode.OTHER_INPUT
      )
  ...
  def update_ar_key_value(
    ...
    if self.quantize_kvcache:
      one_token_key_shaped_for_cache, one_token_key_scale_shaped_for_cache = quantizations.quantize_kv(
        one_token_key_shaped_for_cache, self.kv_quant_axis, ar_cache_axis_names, self.quantize_kvcache)
      one_token_value_shaped_for_cache, one_token_value_scale_shaped_for_cache = quantizations.quantize_kv(
        one_token_value_shaped_for_cache, self.kv_quant_axis, ar_cache_axis_names, self.quantize_kvcache)
